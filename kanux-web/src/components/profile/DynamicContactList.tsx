...
export function DynamicContactList({ contacts, onChange, label = "Contacts"
}: { contacts: Contact[]; onChange: (contacts: Contact[]) => void; label?: string; }) {
  const addContact = () => {
  ...
  const updateContact = (id: string, field: "type" | "value", newValue: string) => {
    onChange(
              ...
              <div key={contact.id} className="flex gap-2 items-start">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <Input
                    ...
                    placeholder={CONTACT_RULES[contact.type]?.placeholder|| "add contact"}
