...
export function DynamicSkillList({ skills, availableCategories, localSkills, addSkillLocal, removeSkillLocal, updateSkillLocal, label = "Skills" }: {
  skills: Skill[];
  availableCategories: Array<{ id: string; name: string }>;
  localSkills: Skill[];
  addSkillLocal: () => void;
  removeSkillLocal: (id: string) => void;
  updateSkillLocal: (id: string, field: "id_category" | "name" | "level", value: string) => void;
  label?: string;
}) {
      ...
      <div className="space-y-3">
        {localSkills.length === 0 ? (
          ...
          localSkills.map((skill: any) => {
            const isTemp = (skill as any)._isNew || skill.id.startsWith('temp-');
            return (
              <div key={skill.id} className="flex gap-2 items-start">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2">
                  <Select
                    ...
                    onChange={(e) => updateSkillLocal(skill.id || '', "id_category", e.target.value)}
                    options={
                  ...
                  <Select
                    ...
                    value={skill.level}
                    onChange={(e) => updateSkillLocal(skill.id || '', "level", e.target.value)}
            ...
            {Object.entries(groupedSkills).map(([categoryId, names]) => (
              <div key={categoryId}>
                <p className="text-xs font-medium text-gray-600 mb-1.5 capitalize">
                  {availableCategories.find(c => c.id === categoryId)?.name || "Uncategorized"}
                </p>
