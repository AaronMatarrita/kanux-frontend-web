...
import { TalentPlan, subscriptionsService } from "@/services/subscriptions.service";
...
export default function TalentBilling() {
  ...
  const [talentPlan,setTalentPlan] = useState<TalentPlan[]>([])
  const [loading, setLoading] = useState<Boolean>(true);

  const getData = async ()=>{
      try{
        const responsePlans= await subscriptionsService.getAllTalentPlans();
        setTalentPlan(responsePlans);
      }catch(error){
