...
export function CreateAboutCompany() {
  ...
  const validate = () => {
    ...
    if (!companyAbout.location) newErrors.location = "Location is required";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      try {
        ...
        setServerError(
          "There was an error registering the company. Please try again.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="w-full">
      ...
      {/* details */}
      <div className="mb-8">
        <p className="text-left font-bold text-foreground text-sm">
          COMPANY DETAILS
        </p>
      </div>
