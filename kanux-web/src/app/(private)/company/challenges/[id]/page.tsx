...
export default function ChallengeDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadChallenge = async () => {
      try {
        ...
        console.error(err);
        setError("No se pudo cargar el detalle del challenge.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (error || !challenge) {
    return (
      <div className="py-8">
        <ErrorAlert
          message={error ?? "Challenge no encontrado"}
  ...
  return (
    <div className="mx-auto py-8 px-8 max-w-6xl">
      {/* Back */}
      <button
        onClick={() => router.back()}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
          <StatCard icon={FileText} label="Tipo" value={challenge.type ?? ""} />
          <StatCard
            icon={Clock}
            label="Duración"
